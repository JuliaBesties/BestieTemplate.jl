...
`only(:feature, path)` regenerates a specific subset of template files without re-running the full interactive setup. Each feature is defined by a single `_only_spec` method in `src/friendly.jl`:
...
1. Add a `_only_spec(::Val{:my_feature})` method in `src/friendly.jl`
...
4. Optionally add a convenience alias: `my_feature(args...; kwargs...) = only(:my_feature, args...; kwargs...)`
5. Add tests in `test/test-only.jl` using the `OnlyHelpers` snippet helpers:
