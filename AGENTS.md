...
The answers file keeps the `_commit` and `_src_path` it already had: `add_feature` saves those two lines and restores them after the copier run (`_copier_bookkeeping` / `_restore_copier_bookkeeping` in `src/utils.jl`, mirrored in the Python package). Copier stamps both with the version it rendered from, but a feature applies a subset of the template, so advancing `_commit` told the next `update` to diff from a version the package never fully had and skip everything in between, silently (#626). The answers themselves are still recorded. Both implementations take `preserve_template_version` (default `true`) to opt out.
