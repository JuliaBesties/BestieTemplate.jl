...
Each feature is defined by an entry in `features.toml` at the repo root (single source of truth, also intended for non-Julia interfaces; schema and semantics in `design/02-features-toml.md`):
