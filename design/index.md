...
1. [ ] **`features.toml` at repo root** with the generic schema and `schema_version`; migrate feature definitions (including descriptions) out of `src/friendly.jl`; Julia's `_add_feature` reads the TOML (behavior-preserving — the existing `test/test-add-feature.jl` suite is the safety net).
