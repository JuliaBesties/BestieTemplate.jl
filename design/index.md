...
4. [x] **SKILL** (moved ahead of the API frontends — highest value for the LLM user story): `skills/bestie-features/SKILL.md` teaches agents the CLI. Playable before the PyPI release via `uvx --from 'git+https://github.com/JuliaBesties/BestieTemplate.jl@api#subdirectory=python' bestie ...` (verified end-to-end, including the registry fetch at `--ref api`).
5. [ ] **FastAPI over L2** (`bestie serve`, localhost-only) — doubles as an executable contract test for L2's schemas.
6. [ ] **MCP server (features-only)**; add an "asking Bestie for features" section to the generated `AGENTS.md` template so every Bestie package becomes agent-upgradable.
7. [ ] **Freezer — revisit deliberately**: question introspection (pending an upstream copier discoverability conversation), guessing, webapp GUI.
8. [ ] **End-state**: Julia's `add_feature` becomes a veneer over the Python L2 once parity has held across a few release cycles.

Steps 1–3 alone deliver the `uv run --with bestie-template bestie add-feature ...` user story; 5 and 6 are each roughly a day on top of a stable L2.
